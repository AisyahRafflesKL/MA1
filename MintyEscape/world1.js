class world1 extends Phaser.Scene {
  ...
  preload() {
    ...
    // Load images without frameWidth/frameHeight for tilesets
  ...
  create() {
    ...
    // Load tilesets with explicit margin and spacing
    let groundTile = map.addTilesetImage(
      "floor-m1",
      "groundTile",
      32,
      32,
    ...
    let ceiling = map.addTilesetImage("ceiling-m1", "ceiling", 32, 32, 0, 0);
    let bg1 = map.addTilesetImage("bg1-m1", "bg1", 32, 32, 0, 0);
    let bg2 = map.addTilesetImage("bg2-m1", "bg2", 32, 32, 0, 0);
    let bg3 = map.addTilesetImage("bg3-m1", "bg3", 32, 32, 0, 0);
    let signs = map.addTilesetImage("signs-m1", "signs", 32, 32, 0, 0);
    let colorOverlay = map.addTilesetImage(
      "colorOverlay-m1",
      "colorOverlay",
      32,
      32,
    ...
    let carts = map.addTilesetImage("carts-m1", "carts", 32, 32, 0, 0);
    ...
    // Keep the mint animation playing
    ...
    // Continue with controlreminderAnim...
    this.anims.create({
    ...
    this.mintIcon = this.add.sprite(440, 150, "excelMint").setScrollFactor(0);
    this.mintIcon.setScrollFactor(0);
    this.mintIcon.setDepth(1000);
    this.mintIcon.play("mintJump"); // Ensure animation is set
    this.mintIcon.setScale(1.2);
    ...
    // unavailable restart level in tuto
    this.restartText = this.add
      .text(300, 200, "Restart Level unavailable in tutorial", {
        fontSize: "19px",
        fontFamily: "Hitchcut",
        fill: "#ffffff",
        backgroundColor: "red",
    ...
    // Guide button click handler (keep only one)
  ...
  update() {
    ...
    // Check distance from trigger and hide texts if player is far enough
    if (this.nextLevelTrigger && this.mintRequirementText.visible) {
      const distance = Phaser.Math.Distance.Between(
          this.player.x, this.player.y,
          this.nextLevelTrigger.x, this.nextLevelTrigger.y
      );
      
      if (distance > 100) {
          if (!this.hideTextTimer) {
              this.hideTextTimer = this.time.delayedCall(1000, () => {
  ...
  resetCollectables() {
    ...
    // Reset score text to 0/2
    this.scoreText.setText("0/2");
  }
