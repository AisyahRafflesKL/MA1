class world2 extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("forestImg", "assets/forestBg.png");
    this.load.image("mountainImg", "assets/mountainBg.png");
    ...
    this.load.image("trigger", "path/to/transparent.png");
  ...
  create() {
    ...
    // let groundTiles = map.addTilesetImage("TX-Village-Props", "villagePropsImg");
    // let propsTiles = map.addTilesetImage("Tileset", "swampImg")
    //let Tiles = map.addTilesetImage("TX Village Props", "villagePropsImg")
    let groundTiles = map.addTilesetImage("Tileset", "swampImg")
    let prop1Tiles = map.addTilesetImage("TX Village Props", "villagePropsImg")
    let mountainTiles = map.addTilesetImage("mountainBg", "mountainImg")
    //let forestTiles = map.addTilesetImage("TX Village Props", "villagePropsImg")
    let forestTiles = map.addTilesetImage("forestBg", "forestImg")
    ...
    let bg4Tiles = map.addTilesetImage("bg4", "bg4Img")
    let bg5Tiles = map.addTilesetImage("bg5", "bg5Img")
    let bg6Tiles = map.addTilesetImage("bg6", "bg6Img")

    
    // Step 5  create an array of tiles

    // let tilesArray = [groundTiles, prop1Tiles, prop2Tiles, prop3Tiles, prop4Tiles, prop5Tiles, prop6Tiles, prop7Tiles, prop8Tiles]
    let tilesArray = [groundTiles, prop1Tiles, mountainTiles, forestTiles, bg1Tiles, bg2Tiles, bg3Tiles, bg4Tiles, bg5Tiles, bg6Tiles]


    // Step 6  Load in layers by layers

    // layers tiles
this.groundLayer = map.createLayer("mountainTile", tilesArray, 0,0)
this.prop1Layer = map.createLayer("forestTile", tilesArray, 0,0)
...
this.prop2Layer = map.createLayer("groundTile", tilesArray, 0,0)
this.prop3Layer = map.createLayer("flowerTile", tilesArray, 0,0)
this.prop4Layer = map.createLayer("bushesTile", tilesArray, 0,0)
this.prop5Layer = map.createLayer("wireTile", tilesArray, 0,0)
//this.propsLayer = map.createLayer("waterTile", tilesArray, 0,0)
//this.prop6Layer = map.createLayer("treeTile", tilesArray, 0,0)
this.prop7Layer = map.createLayer("rocksTile", tilesArray, 0,0)
this.prop8Layer = map.createLayer("signTile", tilesArray, 0,0)
...
this.cameras.main.startFollow(this.player, true, 0.2, 0.2);
    ...
    this.anims.create({
      ...
      frames: this.anims.generateFrameNumbers("excelMint", { start: 0, end: 5 }), // Adjust start and end based on your spritesheet
    ...
    this.prop2Layer.setCollisionByExclusion([-1]);
    this.physics.add.collider(this.player, this.prop2Layer)
...
this.anims.create({
  key: "canonIdle",
  frames: this.anims.generateFrameNumbers("ciggarCanon", { start: 14, end: 0 }),
  frameRate: 7,
  repeat: -1
});
...
this.anims.create({
  key: "smokeBulletAnim",
  frames: this.anims.generateFrameNumbers("smokeBullet", { start: 5, end: 0 }),
  frameRate: 5,
  repeat: -1
});
...
this.musicController = new MusicController(this);
  ...
  update() {
    ...
    this.ciggarCanon.children.iterate((canon) => {
        if (time > (canon.lastFired || 0) + 6100) {  // Fire every 5 seconds
        ...
        if (this.player.y > this.physics.world.bounds.height) {
          console.log("Player fell! Triggering fallDamage.");
          this.fallDamage();
        }
        if (this.cursors.left.isDown) {
    ...
    if (Phaser.Input.Keyboard.JustDown(this.rKey)) {
      ...
      if (currentTime - this.lastRKeyTime < 500) {  // 500ms window for double press
          // Double press detected - restart at spawn
          this.player.setPosition(300, 700);  // Adjust coordinates to your spawn point
