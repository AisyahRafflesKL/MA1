class world3 extends Phaser.Scene {
  ...
  create() {
    ...
    // Remove mountainTiles line
    let forestTiles = map.addTilesetImage(
      "TX-Village-Props",
      "villagePropsImg"
    );
    ...
    let tilesArray = [
      groundTile,
      prop1Tiles,
      // Remove mountainTiles from array
      forestTiles,
      bg1Tiles,
      bg2Tiles,
      bg3Tiles,
    ...
    this.forestTiles = map.createLayer("fruits", tilesArray, 0, 0);
    ...
    //this.prop4Layer.setCollisionByProperty({ slippery: true });
    ...
    this.anims.create({
      ...
      frames: this.anims.generateFrameNumbers("smokeBullet", {
        start: 0, // Changed start from 5 to 0
        end: 2, // Changed end to 2
      }),
      frameRate: 5,
      repeat: -1,
    });
    ...
    this.nextLevelTrigger = this.physics.add.staticSprite(4558, 547, "trigger");
   ...
   nextLevel(player, trigger) {
    ...
    if (this.score >= 6) {
      ...
      this.restartText.setText("[R] x2 to restart level");
  ...
  showSpeechBubble(player, npc) {
    ...
    this.speechBubble.x = npc.x;
    this.speechBubble.y = npc.y - 100;
  }
