class storyboardW3 extends Phaser.Scene {
    constructor() {
        super({ key: 'storyboardW3' });
        this.currentPage = 1;
        this.totalPages = 11;
    ...
    create() {
        ...
        console.log('*** storyboardW3 scene');
...
this.showPage(1);
        ...
        this.leftArrow = this.add.text(200, game.config.height - 100, '<', {
            fontSize: '35px',
            fontFamily: 'Hitchcut',
            fill: '#ffffff',
        ...
        this.rightArrow = this.add.text(game.config.width - 200, game.config.height - 100, '>', {    fontSize: '35px',
            fontFamily: 'Hitchcut',
            fill: '#ffffff',
            backgroundColor: '#1A3449',
            padding: { x: 15, y: 5 }
...
this.closeButton = this.add.text(game.config.width/2, game.config.height - 100, 'Close', {
    fontSize: '25px',
    fontFamily: 'Hitchcut',
    fill: '#ffffff',
    backgroundColor: '#1A3449',
...
this.closeButton.on('pointerdown', () => {
    ...
    showPage(pageNumber) {
        ...
        if (pageNumber === 1) {
            this.currentPageImage = this.add.image(game.config.width/2, game.config.height/2, 'storyboard01');
        } else if (pageNumber === 2) {
            this.currentPageImage = this.add.image(game.config.width/2, game.config.height/2, 'storyboard02');
        } else if (pageNumber === 3) {
            this.currentPageImage = this.add.image(game.config.width/2, game.config.height/2, 'storyboard03');
        } else if (pageNumber === 4) {
            this.currentPageImage = this.add.image(game.config.width/2, game.config.height/2, 'storyboard04');
            // Add ExcelMint sprite
            this.mintSprite = this.add.sprite(game.config.width/2 + 300, game.config.height/2 -240, 'excelMint')
                .setScale(2.5)
                .play('mintJump');
        } else if (pageNumber === 5) {
            this.currentPageImage = this.add.image(game.config.width/2, game.config.height/2, 'storyboard05');
        // Add ExcelMint sprite
        this.mintSprite = this.add.sprite(game.config.width/2 - 160, game.config.height/2 + 280, 'excelMint')
        .setScale(3)
        .play('mintJump');
        } else if (pageNumber === 6) {
            this.currentPageImage = this.add.image(game.config.width/2, game.config.height/2, 'storyboard06');
        } else if (pageNumber === 7) {
            this.currentPageImage = this.add.image(game.config.width/2, game.config.height/2, 'storyboard07');
        } else if (pageNumber === 8) {
            this.currentPageImage = this.add.image(game.config.width/2, game.config.height/2, 'storyboard08');
        } else if (pageNumber === 9) {
            this.currentPageImage = this.add.image(game.config.width/2, game.config.height/2, 'storyboard09');
        } else if (pageNumber === 10) {
            this.currentPageImage = this.add.image(game.config.width/2, game.config.height/2, 'storyboard10');
        } else if (pageNumber === 11) {
            this.currentPageImage = this.add.image(game.config.width/2, game.config.height/2, 'storyboard11');
        }
